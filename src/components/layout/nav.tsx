...
export function Nav({ className, ...props }: React.HTMLAttributes<HTMLElement>) {
  ...
  return (
    <>
      <nav className={cn('fixed top-0 z-50 w-full border-b bg-card py-5', className)} {...props}>
        <div className='mx-auto flex max-w-screen-xl items-center justify-between px-6'>
          ...
          <div className='flex items-center space-x-5'>
            <svg className='h-6 w-6 text-primary lg:hidden' onClick={toggleMenu} fill='none' stroke='currentColor' viewBox='0 0 24 24' xmlns='http://www.w3.org/2000/svg'>
      ...
      <div className={`fixed inset-0 z-50 block bg-background p-5 transition-all lg:hidden ${!isMenuOpen && '-translate-x-[100%]'}`}>
        <div>
          <div className='flex justify-end'>
            <svg className='h-6 w-6 cursor-pointer text-primary' onClick={toggleMenu} fill='none' stroke='currentColor' viewBox='0 0 24 24' xmlns='http://www.w3.org/2000/svg'>
